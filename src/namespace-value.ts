...
export function shouldBlankModule(node: ModuleDeclarationNode): boolean {
    if (node.kind === "global") return true;
    if (node.id.type === "Literal") return true;
    return (
...
function statementHasValue(statement: Node): boolean {
    switch (statement.type) {
        ...
        case "TSModuleDeclaration":
            if (statement.kind === "global") return true;
...
function exportDeclarationHasValue(
    ...
    if (declaration.type === "TSImportEqualsDeclaration") return true;
