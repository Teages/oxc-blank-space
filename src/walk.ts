import { type Comment, type Node, type Program, visitorKeys } from "oxc-parser";
...
function visitNode(blanker: Blanker, node: Node): VisitResult {
    switch (node.type) {
        ...
        case "TSIndexSignature":
            ...
            return VISIT_BLANKED;
        case "CatchClause": {
            if (node.param) visitPattern(blanker, node.param);
...
function visitChildren(blanker: Blanker, node: Node): VisitResult {
    ...
    if (!keys || keys.length === 0) return VISIT_JS;
    ...
    if (children.length === 0) return VISIT_JS;
